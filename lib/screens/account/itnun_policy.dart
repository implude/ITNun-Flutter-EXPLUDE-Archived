...
class PrivatePolicy extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const PolicyBox(
      title: "개인정보처리방침",
      description: """
      < EXPLUDE >('https://github.com/implude/ITNun-Flutter-EXPLUDE'이하 '잇는')은(는) 「개인정보 보호법」 제30조에 따라 정보주체의 개인정보를 보호하고 이와 관련한 고충을 신속하고 원활하게 처리할 수 있도록 하기 위하여 다음과 같이 개인정보 처리방침을 수립·공개합니다.
...
< EXPLUDE >('https://github.com/implude/ITNun-Flutter-EXPLUDE'이하 '잇는')은(는) 다음의 목적을 위하여 개인정보를 처리합니다. 처리하고 있는 개인정보는 다음의 목적 이외의 용도로는 이용되지 않으며 이용 목적이 변경되는 경우에는 「개인정보 보호법」 제18조에 따라 별도의 동의를 받는 등 필요한 조치를 이행할 예정입니다.
...
① < EXPLUDE >은(는) 법령에 따른 개인정보 보유·이용기간 또는 정보주체로부터 개인정보를 수집 시에 동의받은 개인정보 보유·이용기간 내에서 개인정보를 처리·보유합니다.
...
보유근거 : 원활한 서비스 제공
관련법령 :
예외사유 : 사용자가 회원을 탈퇴해 정보 파기를 원하는 경우, 사용자가 마지막으로 서비스를 이용한 지 1년이 지나 휴면 계정으로 전환된 후 1년이 지난 경우


제3조(처리하는 개인정보의 항목)

① < EXPLUDE >은(는) 다음의 개인정보 항목을 처리하고 있습니다.

1< 홈페이지 회원가입 및 관리 >
필수항목 : 이메일, 비밀번호, 로그인ID, 이름, 서비스 이용 기록
선택항목 : 직업, 교급, 지역, 학과/전공, 창업 준비 여부


제4조(개인정보의 파기절차 및 파기방법)


① < EXPLUDE > 은(는) 개인정보 보유기간의 경과, 처리목적 달성 등 개인정보가 불필요하게 되었을 때에는 지체없이 해당 개인정보를 파기합니다.

② 개인정보 파기의 절차 및 방법은 다음과 같습니다.
1. 파기절차
< EXPLUDE > 은(는) 파기 사유가 발생한 개인정보를 선정하고, < EXPLUDE > 의 개인정보 보호책임자의 승인을 받아 개인정보를 파기합니다.
...
① 정보주체는 EXPLUDE에 대해 언제든지 개인정보 열람·정정·삭제·처리정지 요구 등의 권리를 행사할 수 있습니다.

② 제1항에 따른 권리 행사는EXPLUDE에 대해 「개인정보 보호법」 시행령 제41조제1항에 따라 서면, 전자우편, 모사전송(FAX) 등을 통하여 하실 수 있으며 EXPLUDE은(는) 이에 대해 지체 없이 조치하겠습니다.
...
⑥ EXPLUDE은(는) 정보주체 권리에 따른 열람의 요구, 정정·삭제의 요구, 처리정지의 요구 시 열람 등 요구를 한 자가 본인이거나 정당한 대리인인지를 확인합니다.



제7조(개인정보의 안전성 확보조치에 관한 사항)

< EXPLUDE >은(는) 개인정보의 안전성 확보를 위해 다음과 같은 조치를 취하고 있습니다.
...
3. 해킹 등에 대비한 기술적 대책
<EXPLUDE>('잇는')은 해킹이나 컴퓨터 바이러스 등에 의한 개인정보 유출 및 훼손을 막기 위하여 보안프로그램을 설치하고 주기적인 갱신·점검을 하며 외부로부터 접근이 통제된 구역에 시스템을 설치하고 기술적/물리적으로 감시 및 차단하고 있습니다.
...
EXPLUDE 은(는) 정보주체의 이용정보를 저장하고 수시로 불러오는 ‘쿠키(cookie)’를 사용하지 않습니다.
...
< EXPLUDE > 은(는) ｢개인정보 보호법｣ 제15조제3항 및 제17조제4항에 따라 ｢개인정보 보호법 시행령｣ 제14조의2에 따른 사항을 고려하여 정보주체의 동의 없이 개인정보를 추가적으로 이용·제공할 수 있습니다.
이에 따라 < EXPLUDE > 가(이) 정보주체의 동의 없이 추가적인 이용·제공을 하기 위해서 다음과 같은 사항을 고려하였습니다.
...
class AgreeOfCollectNecessary extends StatelessWidget {
  const AgreeOfCollectNecessary({Key? key}) : super(key: key);
...
- 개인정보의 이용 목적 : 수집된 개인정보를 원활한 사용자별 서비스 제공을 위해 사용하며, 목적 외의 용도로는 사용하지 않습니다. 

□ 개인정보의 보관 및 이용 기간
- 귀하의 개인정보를 다음과 같이 보관하며, 수집 및 이용 동의일로부터 본인이 회원을 탈퇴해 개인정보 파기를 요구하거나, 미사용 기간이 1년이 넘어 휴먼 계정으로 전환된 후 1년이 지난 경우 [개인정보 보호법] 제 21조에 따라 처리합니다.  

※귀하는 개인정보 수집에 대한 동의를 거부할 권리가 있습니다. 단 비동의시 서비스 이용이 제한됩니다.""");
  }
}

class AgreeOfCollectOptional extends StatelessWidget {
  const AgreeOfCollectOptional({Key? key}) : super(key: key);
...
□ 개인정보의 보관 및 이용 기간
- 수집 및 이용 동의일로부터 본인이 회원을 탈퇴해 개인정보 파기를 요구하거나, 미사용 기간이 1년이 넘어 휴먼 계정으로 전환된 후 1년이 지난 경우 [개인정보 보호법] 제 21조에 따라 처리합니다.   

※귀하는 개인정보 수집에 대한 동의를 거부할 권리가 있습니다. 단 비동의시 일부 서비스의 원활한 제공이 어려울 수 있습니다.""",
    );
  }
}
